...
4. อ่านตาม "ความหมายพื้นฐาน" ของทิศที่ให้ไว้กับไพ่แต่ละใบ — ตั้งตรง = พลังของไพ่ใบนั้นแสดงออกเต็มที่ (ไพ่หนักอย่างหอคอย สิบดาบ สามดาบ ตั้งตรงก็คือหนักเต็มที่ ไม่ใช่ด้านบวก) ; กลับหัว = อ่านจากความหมายกลับหัวของใบนั้น ห้ามอ่านเหมือนตั้งตรง — กลับหัว/ตั้งตรงต้องเปลี่ยนคำทำนายจริง
...
TXT;
    ...
    private static function cardBlock(Reading $reading): string
    {
        ...
        $key   = TarotSpreads::keyFromType($reading->type);
        $asks  = $key ? array_column(TarotSpreads::positions($key), 'asks') : [];
        // 🗓️ (2026-09-21) 12 เดือน: ชื่อเดือนจริงต่อตำแหน่ง — ทางนี้ (แอพ / ทางสำรอง) เดิมเห็นแค่ "เดือนที่ N"
        //    แม่หมอจึงพูดชื่อเดือนเองไม่ได้ หรือเดาเดือนเริ่มต้นผิด (ชุดเดียวกับที่ส่ง `months` ทางโปรไฟล์)
        $months = $key === 'year' ? FortuneAiService::yearMonths($reading) : [];

        return $cards->map(function ($pc) use ($asks, $months) {
            $card     = $pc->card;
            $reversed = (bool) $pc->reversed;
            ...
            $month    = $months[$pc->position - 1] ?? null;

            $head = "ตำแหน่งที่ {$pc->position} — {$pc->position_label}" . ($month ? " ({$month})" : '');
            if ($ask) {
